...
public class HauntedOccasion extends ConfigClass implements Occasion {
    ...
    public boolean start() {
        try {
            ...
            int radius = (int) (world_border / 2.1);
            for (Team team : teamManager.getTeams()) {
                ...
                int x = (random.nextBoolean() ? 1 : -1) * random.nextInt(radius);
                int z = (random.nextBoolean() ? 1 : -1) * random.nextInt(radius);
                loc.add(x, 0, z);
                ...
                while (loc.getBlock().getType().equals(Material.AIR) || loc.getBlock().getType().equals(Material.WATER)) {
                    ...
                    loc.add((random.nextBoolean() ? 1 : -1) * radius, 0, (random.nextBoolean() ? 1 : -1) * radius);
                    loc = loc.getWorld().getHighestBlockAt(loc).getLocation().clone();
                }
                ...
                for (UUID uid : team.getPlayers()) {
                    ...
                    player.teleport(loc.getWorld().getHighestBlockAt(loc.clone().add(random.nextInt(team_teleport_radius), 0, random.nextInt(team_teleport_radius))).getLocation().clone().add(0, 1, 0));
            ...
            Utils.getLogger().log(Logger.LogLevel.ERROR, ex);
        }
